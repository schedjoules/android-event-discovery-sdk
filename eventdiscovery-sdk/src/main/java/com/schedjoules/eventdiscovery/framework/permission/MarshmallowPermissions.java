...
import com.schedjoules.eventdiscovery.framework.utils.ManifestPermissionStrings;
...
final class MarshmallowPermissions implements AppPermissions
{
    ...
    private static final class MarshmallowPermission implements Permission
    {
        ...
        @Override
        public boolean isGrantable(Activity activity)
        {
            // the permission can be granted at runtime if it's in the manifest and either not requested at all so far or if Android suggests to show
            // the permission rationale (in which case the user didn't decline with "don't ask again").
            return new FilteredIterator<>(new ManifestPermissionStrings(mContext).iterator(), new AnyOf<>(mName)).hasNext()
                    && (!mSharedPreferences.contains(mName) || ActivityCompat.shouldShowRequestPermissionRationale(activity, mName));
        }
